...
def test_record_rejects_unauthorized_not_applicable(tmp_path: Path) -> None:
    ...
    args = argparse.Namespace(
        control="PR-001", status="NOT_APPLICABLE", output=tmp_path / "x.json",
        details="", changed_file=["README.md"]
    )
...
def test_aggregate_fails_for_missing_duplicate_failure_and_malformed(tmp_path: Path) -> None:
    controls = load_catalogue(CATALOGUE)
    (tmp_path / "one.json").write_text(json.dumps({"schema_version": 1, "control_id": "REL-001", "status": "FAIL"}))
    (tmp_path / "two.json").write_text(json.dumps({"schema_version": 1, "control_id": "REL-001", "status": "PASS"}))
...
def test_release_aggregation_passes_with_complete_evidence(tmp_path: Path) -> None:
    ...
    for control_id, item in controls.items():
        if item["lane"] == "release":
            (tmp_path / f"{control_id}.json").write_text(json.dumps({
                "schema_version": 1, "control_id": control_id, "status": "PASS"
            }))
